...
app.post('/additem', (req, res) => {
    // Get value from form by using "req.body.<name give in input tag of .html file>"
    const itemname = req.body.create_item_name;
    const itemcost = req.body.create_item_price;
    // var connect = createConnection();
    // console.log(itemname, itemcost);

    var sqlquery = "Insert into itemtable (itemName, itemPrice) VALUES (?, ?)"
    createConnection().query(sqlquery, [itemname, itemcost], (e, results) => {
        if (e) {
...
app.post('/additem', (req, res) => {
    ...
    var data = req.body;
    console.log(data);
    // res.status(200).send({ message: 'Data Received at Server Side' });
    // Get value from form by using "req.body.<name give in inpit tag of .html file>"
    //const itemname = req.body.create_item_name;
    //const itemcost = req.body.create_item_price;
    // var connect = createConnection();
    // console.log(itemname, itemcost);

    var query = util.format("Insert into itemtable values('%s','%s','%s')", data.itemId, data.itemName, data.itemCost);
    console.log(query)
        // var sqlquery = "Insert into itemtable (itemName, itemPrice) VALUES (?, ?)"
    createConnection().query(query, (e, results) => {
        if (e) {
            console.log('Failed to insert new Entry' + e);
            res.sendStatus(500); // Internal Server Error
            return;
